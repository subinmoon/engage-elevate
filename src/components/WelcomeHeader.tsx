...
const greetingMessages = ["오늘은 무엇이 궁금하세요?", "무엇을 도와드릴까요?", "오늘 하루도 화이팅! 💪", "궁금한 것이 있으시면 물어보세요!", "좋은 하루 되세요! ☀️", "무엇이든 질문해 주세요!", "오늘도 좋은 일만 가득하길! ✨"];
const WelcomeHeader = ({
  userName = "현민",
  onSelectAction
}: WelcomeHeaderProps) => {
  ...
  return <div className="w-full">
      ...
      <div className="flex gap-1.5 overflow-x-auto pb-0.5 sm:pb-0 sm:flex-wrap sm:gap-2 scrollbar-hide -mx-1 px-1">
        {actions.map(action => <button key={action.id} onClick={() => handleActionClick(action)} className="bg-card border border-border rounded-full py-1 sm:py-2 px-2.5 gap-1 sm:gap-2 transition-all duration-200 hover:shadow-soft hover:bg-muted/50 active:scale-[0.98] shrink-0 sm:px-px flex items-center justify-center">
